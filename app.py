...
st.markdown("""
    ...
    /* FIX VISIBILITÀ BOX CARICAMENTO */
...
def pagina_ricerca():
    ...
    with st.container():
        ...
        if st.button("AVVIA FASE 1: CENSIMENTO"):
            with st.spinner("Navigazione web attiva su Giustizia Tributaria..."):
                # System Prompt rinforzato contro i rifiuti di ricerca
        ...
        with st.container():
            ...
            if st.button("AVVIA FASE 2: ANALISI DETTAGLIATA"):
                with st.spinner("Analisi giuridica dei precedenti..."):
                    sys = """Sei un Avvocato Tributarista esperto. Naviga su bancadatigiurisprudenza.giustiziatributaria.gov.it. 
                    Il tuo compito è trovare sentenze specifiche e spiegare la loro utilità tecnica per contrastare l'avviso di accertamento. 
                    ...
                    q = f"Cerca sentenze su bancadatigiurisprudenza.giustiziatributaria.gov.it con parametri: Parole '{s_parole}', Grado {s_grado}, Sede {s_sede}, Esito {s_esito}. Fornisci analisi utilità e suggerimenti."
...
def pagina_redazione():
    ...
    if 'vizi' in st.session_state:
        if st.button("GENERA RICORSO FINALE"):
            client = genai.Client(api_key=st.session_state['gemini_key'])
            prompt = f"Redigi un ricorso su modello FATTO/DIRITTO/PQM usando vizi: {st.session_state['vizi']} e analisi sentenze: {st.session_state.get('giur','')}. Integra le sentenze offline se presenti."
